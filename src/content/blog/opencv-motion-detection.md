...
heroImage: '../../assets/opencv.png'
---

## はじめに

こんにちは！今回は、自動ドアや防犯カメラなど、身近なシステムで利用されている**動体検知**の仕組みをPythonとOpenCVを使って**初心者にもわかりやすく解説**していきたいと思います！

また、動体検知のシミュレーションコードを作成したので、実際に動体検知を体感してみましょう！

https://github.com/97kuek/Motion-Detection-App
...
数式で表すと、ある画素 $(x, y)$ における差分値 $Diff(x, y)$ は以下のようになります。

$$
Diff(x, y) = |Frame(x, y) - Background(x, y)|
$$

ここで、$|...|$ は絶対値を表します。この $Diff(x, y)$ があるしきい値 $Th$ を超えた場合、その画素は「動いている物体の一部」と判定されます。

-   $Diff(x, y) > Th \rightarrow$ **動きあり (1)**
-   $Diff(x, y) \le Th \rightarrow$ **動きなし (0)**

### ピクセル値の変化（イメージ）

ある1点のピクセルに注目し、その輝度値（明るさ）の時間変化をグラフにしてみましょう。物体が通り過ぎた瞬間だけ、輝度値が急激に変化することがわかります。

```mermaid
xychart-beta
    title "Pixel Intensity Change over Time"
    x-axis "Time (Frames)" [0, 10, 20, 30, 40, 50, 60]
    y-axis "Intensity (0-255)" 0 --> 255
    line [50, 52, 51, 53, 200, 210, 205, 55, 53, 52, 51, 50, 52]
```

このグラフのように、普段は背景の明るさ（例：50付近）で安定していますが、白い服を着た人が通った瞬間（フレーム20〜30付近）だけ値が跳ね上がります。背景差分法は、この**「平常時からのズレ」**を検知する仕組みです。
...
そこで、**移動加重平均（Running Average）** という考え方を使います。`cv2.accumulateWeighted` 関数がこれにあたります。数式で書くと以下のようになります。

$$
Reference(t) = (1 - \alpha) \cdot Reference(t-1) + \alpha \cdot Frame(t)
$$

- $Reference(t)$: 時刻 $t$ における更新後の背景モデル
- $Reference(t-1)$: 直前の背景モデル
- $Frame(t)$: 現在の入力フレーム
- $\alpha$: **学習率 (Learning Rate)**。$0 < \alpha < 1$ の範囲の値。

```python
cv2.accumulateWeighted(gray, avg, LEARN_RATE)
```

この式は、**「新しい背景 ＝ (1 - 学習率) × 古い背景  ＋  学習率 × 現在の映像」** という意味です。

-   **学習率 $\alpha$ が高い**: 最新のフレーム $Frame(t)$ の影響が強くなります。背景の変化に素早く追従できますが、一時的に止まった物体を背景と誤認識しやすくなります。
-   **学習率 $\alpha$ が低い**: 過去の背景 $Reference(t-1)$ の影響が強くなります。ゆっくりとした変化に強いですが、急な背景変化への対応が遅れます。

この処理を繰り返すことで、「平均的な背景」を表現する、賢い背景モデル `avg` が育っていきます。

## 3. 差分の計算

背景モデルが準備できたら、現在のフレームと背景モデルとの差分を計算します。`cv2.absdiff` 関数は、2つの画像の各ピクセル値の差の絶対値を取ります。これにより、変化があった部分だけが白く浮かび上がる「差分画像」が生成されます。
