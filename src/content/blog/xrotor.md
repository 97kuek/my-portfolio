...
普段は主翼や尾翼の製作がメインで行っているのですが，最近プロペラ設計のサポートのお手伝いをさせて頂きました．

WASAでは，今年は**プロペラ設計ソフトXROTOR**を用いて設計を行うのですが，その際に作成したパラメータ設定自動化ツールを開発したので，その紹介をします．
...
## ゴール設定：XROTORの「物理モデル」を理解する

XROTORは、翼型の複雑な空力特性を、比較的シンプルな2つの**物理モデル**で表現します。私たちの目標は、XFLR5などが出力した多数のデータ点（実験値）に、これらのモデル式が最もきれいに重なる（フィットする）ように、数式中のパラメータ（$G, \alpha_0, k$など）を見つけ出すことです。

#### 1. 揚力モデル（線形モデル）
「失速前、揚力係数 $C_L$ は迎角 $\alpha$ に対して線形（直線的）に変化する」というモデルです。
$$ C_L = G(\alpha - \alpha_0) $$
...
#### 2. 抗力モデル（放物線モデル）
「抗力係数 $C_D$ は、揚力係数 $C_L$ に対して放物線を描く」というモデルです。
$$ C_D = C_{D,min} + k(C_L - C_{L,opt})^2 $$
-   $C_{D,min}$: **最小抗力係数**。放物線の「頂点の高さ」を決めます。
-   $C_{L,opt}$: **最小抗力時の揚力係数**。放物線の「頂点の横位置」を決めます。
-   $k$: **抗力係数**。放物線の「開き具合」を決めます。

![抗力モデル](../../assets/CLCD.png)
*抗力モデル(放物線モデル)*

では、これらのパラメータをデータからどのように求めるのか、そのアルゴリズムを見ていきましょう。

---

## 抗力極曲線（ドラッグポーラ）のパラメータを求める

まず、抗力モデルから解説します。

#### Step 1: 最も信頼できる基準点を探す
...
これで、いきなり3つのパラメータのうち2つが、最も確実な方法で求まりました。この「基準点」を最初に確定させることで、この後の計算が非常に安定します。

#### Step 2: "形"を決めるパラメータ `k` を求める
...
この「なるべく近くを通る直線を引く」処理を、数学的に最も誤差が少なくなるように自動で行ってくれるのが**最小二乗法**です。プログラムでは `np.linalg.lstsq` という関数がこの賢い計算を実行してくれます。

#### Step 3: フィット範囲の賢い選択
ただし、どのデータを使ってこの直線を引くかが重要です。私たちのプログラムでは、Step 1で見つけた最小抗力点 $C_{L,opt}$ を中心に、`|C_L - C_{L,opt}| < 0.4` のように、その近傍のデータだけを厳選します。

これは、翼の性能の根幹である「ドラッグバケットの底」の領域に計算を集中させ、失速領域などのノイズが多いデータの影響を排除するための工夫です。
...
#### Step 1: 最も"直線らしい"領域を探し出す科学的アプローチ
...
ここまで来れば、あとはプログラムに落とし込むのは容易です。私は、この理論に基づいて自動的に近似モデルを作成し、その結果をXFLR5の解析データと比較しながらパラメータを調整し、最終的に自分好みの近似モデルに仕上げられるようなPythonプログラムを作成しました。
