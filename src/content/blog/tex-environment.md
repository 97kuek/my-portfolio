...
Word で数式を書いているとき、「レイアウトが勝手に崩れる」「数式の入力が面倒」とイライラした経験はありませんか？
...
VSCodeにはTeX用の神拡張機能 **LaTeX Workshop** があります。これを導入すれば、ビルドからPDFプレビューまでVSCodeの中だけで完結します。
...
任意のフォルダを作成し、`report.tex` というファイルを作成して、以下のコードをコピペしてみてください。
...
## 6. 実験レポートテンプレート
文法を覚えるより、まずはテンプレートを活用する方が早いです。これを`report.tex`というファイルに保存し、画像を同じフォルダに入れれば、立派なレポートの完成です！
